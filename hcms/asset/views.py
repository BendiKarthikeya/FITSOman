...
from fits.methods import fits_users_with_perms
...
def asset_request_approve(request, req_id):
    ...
    error_response = f'<script>window.location.href = "{homepage_url}";</script>'
...
def asset_allocate_return_request(request, asset_id):
    ...
    permed_users = fits_users_with_perms("asset.change_assetassignment")
    notify.send(
        request.user.employee_get,
