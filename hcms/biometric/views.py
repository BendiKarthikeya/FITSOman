...
from fits.fits_settings import BIO_DEVICE_THREADS
from fits.settings import TIME_ZONE
...
@install_required
@permission_required("biometric.delete_biometricemployees")
def delete_fits_cosec_user(request, user_id, device_id):
    """
...
def map_biometric_users(request, device_id):
    """
    Maps an fits employee to a biometric user on a specified biometric device.
    """
    device = BiometricDevices.find(device_id)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                ...
                return render(
                    request, "biometric_users/dahua/add_dahua_user.html", 
                )
...
def dahua_biometric_attendance_logs(device):
    """
    Retrieves logs from a Dahua biometric device and marks attendance in Fits.

    This function fetches biometric logs from the specified device, processes the attendance records,
    and updates the attendance system in Fits. If an employee has an active clock-in record,
