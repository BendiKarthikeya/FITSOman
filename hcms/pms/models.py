...
class KeyResult(FitsModel):
    ...
    history = FitsAuditLog(bases=[FitsAuditInfo])
...
class EmployeeObjective(FitsModel):
    ...
    history = FitsAuditLog(bases=[FitsAuditInfo], related_name="history_set")
    archive = models.BooleanField(default=False)
...
class Comment(models.Model):
    ...
    history = FitsAuditLog(excluded_fields=["comment"], bases=[FitsAuditInfo])
    objects = FitsCompanyManager(
...
class QuestionTemplate(FitsModel):
...
class Question(FitsModel):
    ...
    objects = FitsCompanyManager("template_id__company_id")

    def __str__(self):
        return self.question


class QuestionOptions(FitsModel):
    ...
    objects = FitsCompanyManager("question_id__template_id__company_id")


class Feedback(FitsModel):
...
class EmployeeBonusPoint(FitsModel):
    employee_id = models.ForeignKey(
        Employee,
        on_delete=models.DO_NOTHING,
...
class PerformanceRating(FitsModel):
    """Model for managing performance ratings (1-5 scale) for employees"""

    RATING_CHOICES = (
        (1, _("1 - Unsatisfactory")),
        (2, _("2 - Below Expectations")),
        (3, _("3 - Meets Expectations")),
        (4, _("4 - Exceeds Expectations")),
        (5, _("5 - Outstanding")),
    )
    ...
    period_id = models.ForeignKey(
        Period,
        on_delete=models.CASCADE,
        related_name="performance_ratings",
    ...
    competency_rating = models.IntegerField(
        choices=RATING_CHOICES,
        null=True,
        blank=True,
        verbose_name=_("Competency Rating"),
    )
    collaboration_rating = models.IntegerField(
        choices=RATING_CHOICES,
        null=True,
        blank=True,
        verbose_name=_("Collaboration Rating"),
    )
    innovation_rating = models.IntegerField(
        choices=RATING_CHOICES,
        null=True,
        blank=True,
        verbose_name=_("Innovation Rating"),
    )
    customer_focus_rating = models.IntegerField(
        choices=RATING_CHOICES,
        null=True,
        blank=True,
        verbose_name=_("Customer Focus Rating"),
    )
    comments = models.TextField(
        null=True, blank=True, verbose_name=_("Performance Comments")
    )
    strengths = models.TextField(null=True, blank=True, verbose_name=_("Key Strengths"))
    improvement_areas = models.TextField(
        null=True, blank=True, verbose_name=_("Areas for Improvement")
    )
    development_plan = models.TextField(
        null=True, blank=True, verbose_name=_("Development Plan")
    )

    rating_date = models.DateTimeField(auto_now=True, verbose_name=_("Rating Date"))
    is_finalized = models.BooleanField(default=False, verbose_name=_("Finalized"))

    history = FitsAuditLog(bases=[FitsAuditInfo])
    objects = FitsCompanyManager(related_company_field="period_id__company_id")

    class Meta:
        verbose_name = _("Performance Rating")
        verbose_name_plural = _("Performance Ratings")
        unique_together = ("period_id", "employee_id", "manager_id")
        ordering = ["-rating_date"]

    def __str__(self):
        return f"{self.employee_id.get_full_name()} - {self.period_id.period_name}"

    def get_average_rating(self):
        """Calculate average rating across all competencies"""
        ratings = [
            self.overall_rating,
            self.competency_rating,
            self.collaboration_rating,
            self.innovation_rating,
            self.customer_focus_rating,
        ]
    ...
    def get_rating_display(self):
        ...
        if self.overall_rating:
            return dict(self.RATING_CHOICES).get(self.overall_rating, "Not Rated")
        return "Not Rated"


class TalentCalibration(FitsModel):
    """Model for managing talent calibration sessions"""
    ...
    title = models.CharField(
        max_length=200, verbose_name=_("Calibration Session Title")
    )
    period_id = models.ForeignKey(
        Period,
        on_delete=models.CASCADE,
    ...
    facilitator = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="facilitated_calibrations",
        verbose_name=_("Facilitator"),
    )
    participants = models.ManyToManyField(
        Employee,
        related_name="calibration_participants",
        blank=True,
    ...
    meeting_notes = models.TextField(
        null=True, blank=True, verbose_name=_("Meeting Notes")
    )
    recommendations = models.TextField(
        null=True, blank=True, verbose_name=_("Key Recommendations")
    )
    ...
    created_by = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="calibrations_created",
    ...
    class Meta:
        verbose_name = _("Talent Calibration")
        verbose_name_plural = _("Talent Calibrations")
        ordering = ["-scheduled_date"]

    def __str__(self):
        return f"{self.title} - {self.period_id}"


class CalibrationRating(FitsModel):
    """Model to store calibration results for each employee"""

    TALENT_CATEGORIES = (
        ("top_talent", _("Top Talent")),
        ("high_performer", _("High Performer")),
        ("solid_performer", _("Solid Performer")),
    ...
    calibration_id = models.ForeignKey(
        TalentCalibration,
        on_delete=models.CASCADE,
        related_name="calibration_ratings",
    ...
    performance_rating = models.IntegerField(
        choices=PerformanceRating.RATING_CHOICES,
        null=True,
        blank=True,
        verbose_name=_("Performance Rating (1-5)"),
    )
    potential_rating = models.IntegerField(
        choices=PerformanceRating.RATING_CHOICES,
        null=True,
        blank=True,
        verbose_name=_("Potential Rating (1-5)"),
    )
    ...
    calibrated_rank = models.IntegerField(
        null=True,
        blank=True,
        validators=[MinValueValidator(1)],
        verbose_name=_("Calibrated Rank in Department"),
    )

    succession_ready = models.BooleanField(
        default=False, verbose_name=_("Ready for Succession")
    )
    next_move = models.CharField(
        max_length=200,
        null=True,
        blank=True,
        verbose_name=_("Suggested Next Move/Role"),
    )

    calibration_notes = models.TextField(
        null=True, blank=True, verbose_name=_("Calibration Notes")
    )

    history = FitsAuditLog(bases=[FitsAuditInfo])
    objects = FitsCompanyManager(
        related_company_field="calibration_id__period_id__company_id"
    )

    class Meta:
        verbose_name = _("Calibration Rating")
        verbose_name_plural = _("Calibration Ratings")
        unique_together = ("calibration_id", "employee_id")
        ordering = ["calibrated_rank"]

    def __str__(self):
        return f"{self.employee_id} - {self.calibration_id.title}"

    def get_nine_box_position(self):
        """Get the 9-box position based on performance and potential"""
        if not self.performance_rating or not self.potential_rating:
            return None
        ...
        return positions.get(
            (self.performance_rating, self.potential_rating), "Unknown"
        )
