...
from fits_automations.methods.methods import generate_choices
from fits_automations.methods.serialize import serialize_form
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if feedback.based_on == "employee":
                try:
                    notify.send(
                        User.objects.filter(username="Fits Bot").first(),
                        recipient=feedback.employee_id.employee_user_id,
...
def delete_employee_keyresult(request, kr_id):
    ...
    if request.GET.get("dashboard"):
        return redirect("/pms/dashboard-view")
...
def get_keyresult_data(request):
    ...
    if key_result:
        ...
        if request.GET.get("data-update") == "target_value":
            return HttpResponse(
                '<input type="number" name="target_value" value="" class="oh-input w-100 form-control test test" placeholder="Target Value" id="id_target_value">'
            )
        if request.GET.get("data-update") == "end_date":
            return HttpResponse(
                '<input type="date" name="end_date" value="" class="oh-input w-100 form-control" placeholder="End Date" id="id_end_date">'
            )
...
# Performance Rating Views


@login_required
@permission_required("pms.view_performancerating")
def performance_rating_view(request):
    """View all performance ratings"""
    ...
    return render(
        request,
        "pms/performance_rating_list.html",
        {
            "ratings": page_obj,
            "pd": previous_data,
        },
...
@login_required
@permission_required("pms.add_performancerating")
def performance_rating_create(request):
    """Create a new performance rating"""
    from pms.forms import PerformanceRatingForm

    if request.method == "POST":
        form = PerformanceRatingForm(request.POST)
        if form.is_valid():
            rating = form.save(commit=False)
            rating.manager_id = request.user.employee_get
            rating.save()
            messages.success(request, _("Performance rating created successfully."))
            return redirect("performance-rating-view")
    else:
        form = PerformanceRatingForm()

    return render(
        request,
        "pms/performance_rating_form.html",
        {
            "form": form,
            "title": _("Create Performance Rating"),
        },
    )


@login_required
@permission_required("pms.change_performancerating")
def performance_rating_update(request, rating_id):
    """Update an existing performance rating"""
    from pms.forms import PerformanceRatingForm

    rating = get_object_or_404(PerformanceRating, id=rating_id)

    if request.method == "POST":
        form = PerformanceRatingForm(request.POST, instance=rating)
        if form.is_valid():
            rating = form.save(commit=False)
            rating.save()
            messages.success(request, _("Performance rating updated successfully."))
            return redirect("performance-rating-view")
    else:
        form = PerformanceRatingForm(instance=rating)
    ...
    return render(
        ...
        {
            "form": form,
            "title": _("Update Performance Rating"),
            "rating": rating,
        },
    )
...
@login_required
@permission_required("pms.view_performancerating")
def performance_rating_detail(request, rating_id):
    """View detailed performance rating"""
    rating = get_object_or_404(PerformanceRating, id=rating_id)

    return render(
        request,
        "pms/performance_rating_detail.html",
        {
            "rating": rating,
        },
    )
