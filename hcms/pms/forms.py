...
from base.forms import ModelForm as FitsModelForm
...
class ObjectiveForm(BaseForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["assignees"] = FitsMultiSelectField(
        ...
        self.fields["managers"] = FitsMultiSelectField(
            queryset=Employee.objects.all(),
            widget=FitsMultiSelectWidget(
                filter_route_name="employee-widget-filter",
                filter_class=EmployeeFilter,
    ...
    def clean(self):
        ...
        for field_name, field_instance in self.fields.items():
            if isinstance(field_instance, FitsMultiSelectField):
        ...
        if not managers or managers is None:
...
class FeedbackForm(FitsModelForm):
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        # # Fits multi-select filter for subordinates
        # self.fields["subordinate_id"] = FitsMultiSelectField(
        #     queryset=Employee.objects.all(),
        #     widget=FitsMultiSelectWidget(
        #         filter_route_name="employee-widget-filter",
        #         filter_class=EmployeeFilter,
...
class PeriodForm(FitsModelForm):
    """
...
class BonusPointSettingForm(FitsModelForm):
    """
    ...
    def clean(self):
        ...
        if model in ["pms.models.EmployeeObjective", "pms.models.EmployeeKeyResult"]:
            ...
            if not cleaned_data["bonus_for"] == "Closed":
                raise ValidationError(
                    _("This 'Bonus for' is not in the Model's status")
                )
...
class EmployeeBonusPointForm(FitsModelForm):
    """
...
class EmployeeFeedbackForm(FitsModelForm):
    cols = {"others_id": 12}
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields["others_id"] = FitsMultiSelectField(
            queryset=Employee.objects.filter(employee_work_info__isnull=False),
            widget=FitsMultiSelectWidget(
                filter_route_name="employee-widget-filter",
                filter_class=EmployeeFilter,
    ...
    def clean(self):
        cleaned_data = super().clean()
        if isinstance(self.fields["others_id"], FitsMultiSelectField):
...
class BulkFeedbackForm(FitsModelForm):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields["employee_ids"] = FitsMultiSelectField(
            queryset=Employee.objects.filter(employee_work_info__isnull=False),
            widget=FitsMultiSelectWidget(
                filter_route_name="employee-widget-filter",
                filter_class=EmployeeFilter,
    ...
    def clean(self):
        cleaned_data = super().clean()
        if isinstance(self.fields["employee_ids"], FitsMultiSelectField):
...
class PerformanceRatingForm(FitsModelForm):
    """Form for creating and updating performance ratings"""
    ...
    class Meta:
        ...
        labels = {
            "period_id": _("Performance Period"),
            "employee_id": _("Employee"),
            "overall_rating": _("Overall Rating"),
            "competency_rating": _("Competency Rating"),
            "collaboration_rating": _("Collaboration Rating"),
            "innovation_rating": _("Innovation Rating"),
            "customer_focus_rating": _("Customer Focus Rating"),
            "comments": _("Comments"),
            "strengths": _("Key Strengths"),
            "improvement_areas": _("Areas for Improvement"),
            "development_plan": _("Development Plan"),
        }
        widgets = {
            "comments": forms.Textarea(attrs={"rows": 4}),
            "strengths": forms.Textarea(attrs={"rows": 4}),
            "improvement_areas": forms.Textarea(attrs={"rows": 4}),
            "development_plan": forms.Textarea(attrs={"rows": 4}),
        }

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields["period_id"].queryset = Period.objects.all()
