...
def candidate_delete(request, cand_id):
    ...
    try:
        try:
            ...
            error_msg = _("Candidate not found.")
            ...
            error_msg = _("Cannot delete: candidate is linked to {}.".format(models_verbose_name_str))
            messages.error(request, error_msg)
    except (Candidate.DoesNotExist, OverflowError):
        error_msg = _("Candidate does not exist.")
        messages.error(request, error_msg)

    if is_ajax:
        from django.http import JsonResponse as _JR
        if error_msg:
            return _JR({"success": False, "error": str(error_msg)}, status=400)
...
def get_template(request, obj_id=None):
    ...
    body = ""
    if obj_id:
        body = FitsMailTemplate.objects.get(id=obj_id).body
