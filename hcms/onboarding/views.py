...
from fits.group_by import group_by_queryset as general_group_by
...
def candidates_view(request):
    ...
    mail_templates = FitsMailTemplate.objects.all()
...
def update_offer_letter_status(request):
    ...
    if status not in ["not_sent", "sent", "accepted", "rejected", "joined"]:
...
@login_required
def pre_boarding(request):
    return render(request, "onboarding/pre_boarding.html")


@login_required
def probation_management(request):
    return render(request, "onboarding/probation_management.html")
