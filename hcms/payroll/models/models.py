            ...
            if self.status == "approved" and self.allowance_id is None:
                if self.type == "reimbursement":
                    ...
                    if bonus_points.points >= self.bonus_to_encash:
                        ...
                        request = getattr(
                            fits_middlewares._thread_locals, "request", None
                        )
                        if request:
                            messages.info(
                                request,
...
class ReimbursementrequestComment(FitsModel):
    """
...
class LeaveEncashment(FitsModel):
    """Model for managing leave encashment during employee separation"""
    ...
    leave_count = models.DecimalField(
        max_digits=5, decimal_places=2, verbose_name="Total Leave Days Available"
    )
    leave_type = models.ForeignKey(
        ...
        null=True,
        blank=True,
        verbose_name="Leave Type Encashed",
    )

    # Calculation details
    encashment_date = models.DateField(
        auto_now_add=True, verbose_name="Encashment Date"
    )
    daily_wage = models.DecimalField(
        max_digits=12, decimal_places=2, verbose_name="Daily Wage"
    )
    total_encashment_amount = models.DecimalField(
        ...
        null=True,
        blank=True,
        verbose_name="Total Encashment Amount",
    )

    # Tax and deductions
    tax_deducted = models.DecimalField(
        max_digits=12, decimal_places=2, default=0, verbose_name="Tax Deducted"
    )
    other_deductions = models.DecimalField(
        max_digits=12, decimal_places=2, default=0, verbose_name="Other Deductions"
    )
    net_encashment_amount = models.DecimalField(
        max_digits=12,
        decimal_places=2,
        null=True,
        blank=True,
        verbose_name="Net Encashment Amount",
    )
    ...
    calculated_by = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="leave_encashments_calculated",
    ...
    approved_by = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="leave_encashments_approved",
        verbose_name="Approved By",
    )

    approved_date = models.DateTimeField(
        null=True, blank=True, verbose_name="Approved Date"
    )
    ...
    def __str__(self):
        return f"{self.employee_id.get_full_name()} - {self.leave_count} days"

    def calculate_encashment(self):
        """Calculate encashment amount based on leave days and daily wage"""
        if self.leave_count and self.daily_wage:
            self.total_encashment_amount = float(self.leave_count) * float(
                self.daily_wage
            )
            # Calculate net after deductions
            total_deductions = float(self.tax_deducted) + float(self.other_deductions)
            self.net_encashment_amount = self.total_encashment_amount - total_deductions
            self.status = "calculated"
            self.save()

    def approve_encashment(self, approved_by_employee=None):
        """Approve the encashment"""
        from django.utils import timezone

        self.status = "approved"
        self.approved_date = timezone.now()
        if approved_by_employee:
...
class SalaryRevision(FitsModel):
    """Model for managing employee salary revisions and increments"""
    ...
    current_basic_salary = models.DecimalField(
        max_digits=12, decimal_places=2, verbose_name="Current Basic Salary"
    )
    new_basic_salary = models.DecimalField(
        max_digits=12, decimal_places=2, verbose_name="New Basic Salary"
    )
    increment_percentage = models.DecimalField(
        ...
        null=True,
        blank=True,
        verbose_name="Increment Percentage",
    )

    # Current and new gross
    current_gross_salary = models.DecimalField(
        max_digits=12,
        decimal_places=2,
        null=True,
        blank=True,
        verbose_name="Current Gross Salary",
    )
    new_gross_salary = models.DecimalField(
        max_digits=12,
        decimal_places=2,
        null=True,
        blank=True,
        verbose_name="New Gross Salary",
    )
    ...
    submitted_by = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="salary_revisions_submitted",
        verbose_name="Submitted By",
    )
    submitted_date = models.DateTimeField(
        null=True, blank=True, verbose_name="Submitted Date"
    )
    ...
    rejection_reason = models.TextField(
        null=True, blank=True, verbose_name="Rejection Reason"
    )

    # Additional information
    justification = models.TextField(verbose_name="Justification for Revision")
    new_contract = models.FileField(
        upload_to="salary_revisions/",
        null=True,
        blank=True,
        verbose_name="Updated Contract Document",
    )
    ...
    def __str__(self):
        return f"{self.employee_id.get_full_name()} - {self.revision_reason}"

    def calculate_increment_percentage(self):
        """Calculate increment percentage from current and new salary"""
        if self.current_basic_salary and self.new_basic_salary:
            increment = self.new_basic_salary - self.current_basic_salary
            self.increment_percentage = (increment / self.current_basic_salary) * 100

    def submit_revision(self, submitted_by_employee):
        """Submit the revision for approval"""
        from django.utils import timezone

        self.status = "submitted"
        self.submitted_by = submitted_by_employee
        self.submitted_date = timezone.now()
        self.save()

    def approve_revision(self, approved_by_employee):
        """Approve the revision"""
        from django.utils import timezone
        from django.core.exceptions import ValidationError

        if self.status != "submitted":
            raise ValidationError("Revision must be submitted before approval")

        self.status = "approved"
        self.approved_by = approved_by_employee
        self.approved_date = timezone.now()

        # If effective_from is today or in the past, mark as effective
        from datetime import date

        if self.effective_from <= date.today():
            self.status = "effective"
            # Update the employee's contract with new salary
            self._update_employee_contract()

        self.save()

    def reject_revision(self, rejection_reason, rejected_by_employee=None):
        """Reject the revision"""
        self.status = "rejected"
        self.rejection_reason = rejection_reason
        if rejected_by_employee:
            self.approved_by = rejected_by_employee
        self.save()

    def _update_employee_contract(self):
        """Update employee's active contract with new salary"""
        try:
            ...
            if contract:
                contract.wage = self.new_basic_salary
                contract.save()
        except Exception:
            pass
