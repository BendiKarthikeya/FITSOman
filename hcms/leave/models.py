...
class LeaveType(FitsModel):
    ...
    def leave_type_next_reset_date(self):
        ...
        if self.reset_based == "yearly":
            month, day = (
                int(self.reset_month),
                get_reset_day(int(self.reset_month), self.reset_day),
            )
            reset_date = datetime(
...
class AvailableLeave(FitsModel):
...
class LeaverequestComment(FitsModel):
    """
...
class LeaveAllocationRequest(FitsModel):
    leave_type_id = models.ForeignKey(
        LeaveType, on_delete=models.PROTECT, verbose_name=_("Leave type")
    )
    employee_id = models.ForeignKey(
...
class LeaveallocationrequestComment(FitsModel):
    """
...
if apps.is_installed("attendance"):
    ...
    class CompensatoryLeaveRequest(FitsModel):
        leave_type_id = models.ForeignKey(
...
class LeaveGeneralSetting(FitsModel):
    """
...
if apps.is_installed("attendance"):
    ...
    class CompensatoryLeaverequestComment(FitsModel):
        """
...
class EmployeePastLeaveRestrict(FitsModel):
...
if apps.is_installed("attendance"):
    ...
    class OverrideLeaveRequests(LeaveRequest):
        ...
        #     """
        #     WorkRecords = get_fits_model_class(
