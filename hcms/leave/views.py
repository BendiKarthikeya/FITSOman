...
def leave_request_view(request):
    ...
    if apps.is_installed("recruitment"):
        for leave_request in leave_requests:
            ...
            InterviewSchedule = get_fits_model_class(
...
@login_required
def on_leave_today_count(request):
    """
    Returns a plain-text count of employees on approved leave today.
    Used as an HTMX stat tile on the superuser dashboard.
    """
    count = LeaveRequest.employees_on_leave_today(status="approved").count()
...
if apps.is_installed("attendance"):
    ...
    def delete_compensatory_leave(request, comp_id):
        ...
        """
        try:
            CompensatoryLeaveRequest.objects.get(id=comp_id).delete()
