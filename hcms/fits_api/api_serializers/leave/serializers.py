...
def leave_Validations(self, data):
    ...
    if leave_type_id.require_attachment == "yes" and attachment is None:
...
class LeaveTypeGetCreateSerilaizer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        data.get("carryforward_type")
        data.get("carryforward_max")
        if reset:
            if reset_based is None:
                raise serializers.ValidationError(
                    {"reset_based": ["This field is required."]}
                )
            elif reset_based == "yearly" and reset_month is None:
                raise serializers.ValidationError(
            ...
            elif reset_based == "weekly" and reset_weekday is None:
                raise serializers.ValidationError(
