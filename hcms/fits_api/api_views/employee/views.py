...
from fits.decorators import owner_can_enter
from fits_api.api_decorators.base.decorators import permission_required
...
class DocumentRequestAPIView(APIView):
    ...
    @manager_permission_required("fits_documents.add_documentrequests")
    def post(self, request):
    ...
    @manager_permission_required("fits_documents.change_documentrequests")
    def put(self, request, pk):
        document_request = self.get_object(pk)
...
class DocumentAPIView(APIView):
    ...
    @manager_or_owner_permission_required(
        DocumentRequest, "fits_documents.add_document"
    )
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            try:
                notify.send(
                    ...
                    icon="chatbox-ellipses",
                    api_redirect="/api/employee/documents/",
                )
            except:
                pass
            return Response(serializer.data, status=status.HTTP_201_CREATED)
    ...
    @method_decorator(owner_can_enter("fits_documents.change_document", Employee))
    def put(self, request, pk):
        document = self.get_object(pk)
    ...
    @method_decorator(owner_can_enter("fits_documents.delete_document", Employee))
    def delete(self, request, pk):
        document = self.get_object(pk)
        document.delete()
        return Response(status=status.HTTP_204_NO_CONTENT)
...
class DocumentRequestApproveRejectView(APIView):
    ...
    @manager_permission_required("fits_documents.add_document")
...
# ── ERP / HRMS Integration Endpoints ──────────────────────────────────────────

class EmployeeERPExportView(APIView):
    """
    GET /api/employees/erp-export/
    Returns full employee master data for ERP/HRMS sync.
    Requires: is_staff or 'employee.view_employee' permission.
    Optional query params: ?is_active=true  ?company_id=1
    """

    permission_classes = [IsAuthenticated]

    def get(self, request):
        if not request.user.is_staff and not request.user.has_perm("employee.view_employee"):
            ...
            salary_history = [
                {
                    ...
                    "basic_salary": str(sh.basic_salary),
                    "gross_salary": str(sh.gross_salary) if sh.gross_salary else None,
                    "reason": sh.increment_reason,
                }
                for sh in emp.salary_history.all()[:10]
            ]
            data.append({
                "id": emp.id,
                ...
                "work_permit_number": getattr(emp, "work_permit_number", None),
                "last_appraisal_date": str(emp.last_appraisal_date) if getattr(emp, "last_appraisal_date", None) else None,
                "last_appraisal_rating": getattr(emp, "last_appraisal_rating", None),
                "is_active": emp.is_active,
                "date_joining": str(wi.date_joining) if wi and wi.date_joining else None,
                "contract_end_date": str(wi.contract_end_date) if wi and wi.contract_end_date else None,
                "basic_salary": str(wi.basic_salary) if wi and wi.basic_salary else None,
                ...
                "company": str(wi.company_id) if wi and wi.company_id else None,
                "reporting_manager_id": wi.reporting_manager_id.id if wi and wi.reporting_manager_id else None,
...
class EmployeeERPImportView(APIView):
    """
    POST /api/employees/erp-import/
    Upsert employee master data pushed from an external ERP.
    Matches on email. Creates or updates Employee + EmployeeWorkInformation.
    Requires: is_staff permission.

    Body: { "employees": [ { "email": "...", "employee_first_name": "...", ... } ] }
    """
    ...
    def post(self, request):
        ...
        if not isinstance(employees_data, list):
            return Response({"detail": "Expected a list under 'employees'."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        for idx, emp_data in enumerate(employees_data):
            ...
            try:
                emp, created = Employee.objects.get_or_create(
                    email=email,
                    defaults={
                        "employee_first_name": emp_data.get("employee_first_name", email.split("@")[0]),
                        "phone": emp_data.get("phone", ""),
                    },
                )
                # Update fields
                for field in ["employee_first_name", "employee_last_name", "phone", "dob", "gender",
                              "marital_status", "nationality", "national_id", "passport_number",
                              "work_permit_number", "last_appraisal_date", "last_appraisal_rating"]:
                    if field in emp_data and emp_data[field] is not None:
                ...
                wi = getattr(emp, "employee_work_info", None)
                if wi:
                    for field in ["date_joining", "contract_end_date", "basic_salary"]:
                        if field in emp_data and emp_data[field] is not None:
                            setattr(wi, field, emp_data[field])
                    wi.save()
