...
from fits.filters import FilterSet, filter_by_name
...
class ShiftRequestFilter(FilterSet):
    ...
    def __init__(self, data=None, queryset=None, *, request=None, prefix=None):
        ...
        # Eagerly evaluate any queryset-backed choices to avoid database
        # server-side cursor issues during template rendering (fetchmany
        # on a closed cursor). Converting `choices` to a list forces
        # evaluation while the DB cursor is valid.
        for name, fld in self.form.fields.items():
            try:
                # Accessing .choices and wrapping with list() forces evaluation
                # for ModelChoiceField / QuerySet-backed fields.
                fld.choices = list(fld.choices)
            except Exception:
                # non-iterable or already evaluated fields can be ignored
                pass
...
class WorkTypeRequestFilter(FilterSet):
    ...
    def __init__(self, data=None, queryset=None, *, request=None, prefix=None):
        ...
        for field in self.form.fields.keys():
            self.form.fields[field].widget.attrs["id"] = f"{uuid.uuid4()}"
        # Eagerly evaluate choices to avoid cursor reuse errors during
        # template rendering (similar rationale as above).
        for name, fld in self.form.fields.items():
            try:
                fld.choices = list(fld.choices)
            except Exception:
...
class HolidayFilter(FilterSet):
    ...
    def __init__(self, data=None, queryset=None, *, request=None, prefix=None):
        ...
        self.form.fields[
            "from_date"
        ].label = f"{self.Meta.model()._meta.get_field('start_date').verbose_name} From"
        self.form.fields[
            "to_date"
        ].label = f"{self.Meta.model()._meta.get_field('end_date').verbose_name} Till"
