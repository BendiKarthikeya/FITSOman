...
from fits.filters import FitsPaginator
from fits.group_by import group_by_queryset
from fits.fits_settings import FITS_DATE_FORMATS
from fits.methods import get_fits_model_class
from fits_audit.models import AccountBlockUnblock, HistoryTrackingFields
from fits_documents.forms import (
    DocumentForm,
    DocumentRejectForm,
    DocumentRequestForm,
    DocumentUpdateForm,
...
@login_required
def raise_recruitment(request):
    employee = getattr(request.user, "employee_get", None)

    requests = (
        Recruitment.objects.filter(raised_by=employee)
        .select_related("raised_by")
        .prefetch_related("approvals__approver")
        .order_by("-created_at")
        if employee
        else Recruitment.objects.none()
    )
    ...
    for rec in requests:
        data.append({
            "id": rec.id,
            "raised_by": str(rec.raised_by),
            "created_at": _fmt(rec.created_at),
            "job_position": str(getattr(rec, "job_position", "")),
            "approvals": [
                {
    ...
    context = {
        "requests": requests,
        "requests_json": json.dumps(data)  # 🔥 THIS IS KEY
    }

    return render(
        request,
        "employee/raise_recruitment.html",
        context
    )
...
@login_required
@manager_can_enter("fits_documents.view_documentrequest")
def document_request_view(request):
    ...
    documents = Document.objects.select_related(
        "employee_id",
        "document_request_id",
    ).filter(document_request_id__isnull=False)

    # Check if any filter params (other than pagination) are present
    filter_params = {k: v for k, v in request.GET.items() if k not in ("page", "")}
    if filter_params:
        filtered_docs = filter_class.qs
        filtered_docs = filtered_docs.filter(document_request_id__isnull=False)
        documents = filtered_docs
    else:
        # No filter applied - show all documents visible to this manager
        documents = filtersubordinates(
            request=request,
            perm="fits_documents.view_documentrequest",
            queryset=documents,
        )
...
@login_required
@hx_request_required
@manager_can_enter("fits_documents.add_documentrequest")
def document_request_create(request):
    ...
    form = choosesubordinates(request, form, "fits_documents.add_documentrequest")
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            employees = [
                user.employee_user_id for user in doc_request.employee_id.all()
            ]
...
@login_required
@hx_request_required
@manager_can_enter("fits_documents.change_documentrequest")
...
@login_required
@hx_request_required
@owner_can_enter("fits_documents.view_document", Employee)
...
@login_required
@hx_request_required
@owner_can_enter("fits_documents.add_document", Employee)
...
def document_delete(request, id):
    ...
    try:
        ...
        if not request.user.has_perm("fits_documents.delete_document"):
...
@login_required
@hx_request_required
@manager_can_enter("fits_documents.add_document")
...
@login_required
@manager_can_enter("fits_documents.delete_document")
def document_bulk_delete(request):
    """
    This function is used to bulk-delete documents.
    ...
    if request.method == "POST":
        ...
        if deleted_count:
            messages.success(
                request, _(f"{deleted_count} document(s) deleted successfully")
            )
        return HttpResponse(status=204, headers={"HX-Refresh": "true"})
...
def paginator_qry(qryset, page_number):
    ...
    """
    paginator = FitsPaginator(qryset, get_pagination())
...
def employee_view(request):
    ...
    # Reuse the same EmployeeFilter instance for both filtering and the form
    # display — avoids double-instantiation and its associated DB round-trips.
    employee_filter = EmployeeFilter(request.GET, queryset=queryset)
    filter_obj = employee_filter.qs
    ...
    # Store the employee IDs in the session efficiently
                            ...
                            if parts[-1]:
                                if parts[0] == "employee_work_info":
                                    ...
                                    if (
                                        ...
                                        if (
                                            "employee_work_info__department_id" not in update_fields
                                        ):
                                        ...
                                        if (
                                            "employee_work_info__job_position_id" not in update_fields
                                        ):
                                        ...
                                        if (
                                            "employee_work_info__job_role_id" not in update_fields
                                        ):
...
def employee_filter_view(request):
    ...
    if field != "" and field is not None:
        ...
        # Store the employees in the session efficiently
    ...
    if request.GET.get("is_active") is None:
        filter_obj = EmployeeFilter(
            ...
            queryset=Employee.objects.select_related(
                ...
                "employee_work_info__company_id",
                "employee_user_id",
            ).filter(employee_first_name__icontains=search, is_active=True),
        )
    else:
        filter_obj = EmployeeFilter(
            request.GET,
        ...
        for models in related_models:
            ...
            if field_name:
                ...
                if (
                    ...
                    Recruitment = get_fits_model_class(
                    ...
                    Stage = get_fits_model_class(app_label="recruitment", model="stage")
                    ...
                    OnboardingStage = get_fits_model_class(
                    ...
                    OnboardingTask = get_fits_model_class(
    ...
    for employee in employees:
        for column_value, column_name in selected_columns:
            ...
            if isinstance(value, date):
                try:
                    data = value.strftime(
                        FITS_DATE_FORMATS.get(date_format, "%Y-%m-%d")
                    )
                except Exception:
                    data = str(value)
...
def joining_today_count(request):
    ...
    if apps.is_installed("recruitment"):
        Candidate = get_fits_model_class(app_label="recruitment", model="candidate")
...
def bonus_points_tab(request, emp_id):
    ...
    try:
        ...
        if apps.is_installed("payroll"):
            Reimbursement = get_fits_model_class(
...
def manager_dashboard(request):
    """
    This method is used to display the manager dashboard with consolidated manager features.
    Shows pending approvals, team statistics, and quick links to manager features.
    """
    try:
        manager = request.user.employee_get
    except:
        messages.error(
            request, _("You are not an employee. Cannot access manager portal.")
        )
        return redirect("employee-view")

    # Get manager's subordinates/team members from EmployeeWorkInformation
    ...
    # Try to get leave request counts
    try:
        from leave.models import LeaveRequest

        pending_leaves = LeaveRequest.objects.filter(
            employee_id__in=team_members, status="requested"
        ).count()
        approved_leaves = LeaveRequest.objects.filter(
            employee_id__in=team_members, status="approved"
        ).count()
        context["pending_leaves"] = pending_leaves
        context["approved_leaves"] = approved_leaves
    except:
        context["pending_leaves"] = 0
        context["approved_leaves"] = 0

    # Try to get attendance request counts
    try:
        ...
        context["pending_attendance"] = pending_attendance
    except:
        context["pending_attendance"] = 0

    # Try to get shift request counts
    try:
        pending_shift_requests = ShiftRequest.objects.filter(
            employee_id__in=team_members, status="requested"
        ).count()
        context["pending_shift_requests"] = pending_shift_requests
    except:
        context["pending_shift_requests"] = 0

    # Try to get project task counts
    try:
        from project.models import Task

        assigned_tasks = Task.objects.filter(
            assigned_to_id__in=team_members.values_list("id", flat=True)
        ).count()
        context["assigned_tasks"] = assigned_tasks
    except:
        context["assigned_tasks"] = 0

    return render(
        request,
        "employee/manager/manager_dashboard_enhanced.html",
        context,
    )
