...
# Throttle: run auto-punch-out check at most once per 60 seconds per process
...
class AttendanceMiddleware(MiddlewareMixin):
    ...
    is enabled for their shift. It processes this during each request, but is throttled
    to run at most once per minute to avoid excessive DB queries.
    """

    def process_request(self, request):
        """
        Triggers the `trigger_function` on each request, throttled to once per minute.
        """
        global _last_trigger_time
        now = time.monotonic()
        if now - _last_trigger_time >= _TRIGGER_INTERVAL:
