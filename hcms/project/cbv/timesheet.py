...
from fits_views.cbv_methods import login_required
from fits_views.generic.cbv.views import (
    FitsCardView,
    FitsDetailedView,
    FitsFormView,
    FitsListView,
...
class TimeSheetNavView(FitsNavView):
    """
    Nav bar
    """
...
class TimeSheetList(FitsListView):
    """
...
class TaskTimeSheet(TimeSheetList):
    ...
    def get_queryset(self):
        queryset = FitsListView.get_queryset(self)
        task_id = self.kwargs.get("task_id")
        ...
        if employee_id:
            ...
            if (
                employee
                and employee not in task.task_managers.all()
...
class TimeSheetFormView(FitsFormView):
    """
...
class TimeSheetCardView(FitsCardView):
    """
    For card view
    """
...
class TimeSheetDetailView(FitsDetailedView):
    """
