...
class Project(FitsModel):
    ...
    def save(self, *args, **kwargs):
        is_new, request = (
            self.pk is None,
            getattr(fits_middlewares._thread_locals, "request", None),
        )
...
class ProjectStage(FitsModel):
    ...
    objects = FitsCompanyManager("project__company_id")
...
class TimeSheet(FitsModel):
    ...
    objects = FitsCompanyManager("project_id__company_id")
        ...
        if self.employee_id:
            ...
            if self.task_id:
                ...
                if (
                    ...
                    and employee not in task.project.managers.all()
                    and employee not in task.project.members.all()
                ):
                    raise ValidationError(_("Employee not included in this task"))
            elif self.project_id:
                if (
                    employee not in self.project_id.managers.all()
                    and employee not in self.project_id.members.all()
                ):
