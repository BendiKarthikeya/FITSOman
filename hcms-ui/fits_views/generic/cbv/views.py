...
from fits.filters import FilterSet
from fits.group_by import group_by_queryset
from fits.fits_middlewares import _thread_locals
from fits.signals import post_generic_import, pre_generic_import
from fits_views import models
from fits_views.cbv_methods import (  # update_initial_cache,
    assign_related,
    export_xlsx,
...
from fits_views.forms import DynamicBulkUpdateForm, ToggleColumnForm
...
class FitsListView(ListView):
    ...
    template_name: str = "generic/fits_list_table.html"
    ...
    def serve_import_sheet(self, request, *args, **kwargs):
        ...
        if not self.import_accessibility():
            ...
            return FitsFormView.HttpResponse()
    ...
    def get_context_data(self, **kwargs: Any):
        ...
        # CACHE.get(self.request.session.session_key + "cbv")[FitsListView] = context
        from fits.urls import path, urlpatterns
    ...
    def export_data(self, *args, **kwargs):
        ...
        class FitsListViewResorce(resources.ModelResource):
            """
        ...
        book_resource = FitsListViewResorce()
        ...
        # CSV
            ...
            if pisa is None:
                messages.error(
                    request,
                    _("PDF export is unavailable because xhtml2pdf is not installed."),
                )
                return HttpResponse(
...
class FitsSectionView(TemplateView):
    """
    Fits Template View
    """
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        request = getattr(_thread_locals, "request", None)
        self.request = request
        # update_initial_cache(request, CACHE, FitsListView)
    ...
    template_name = "generic/fits_section.html"
...
@method_decorator(hx_request_required, name="dispatch")
class FitsDetailedView(DetailView):
    """
    HorillDetailedView
    """

    title = "Detailed View"
    template_name = "generic/fits_detailed_view.html"
    header: dict = {
        "title": "Fits",
        "subtitle": "Fits Detailed View",
        "avatar": "",
    }
    body: list = []
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        request = getattr(_thread_locals, "request", None)
        self.request = request
        # update_initial_cache(request, CACHE, FitsDetailedView)
...
@method_decorator(hx_request_required, name="dispatch")
class FitsTabView(TemplateView):
    """
    FitsTabView
    """

    view_id: str = get_short_uuid(3, "htv")
    template_name = "generic/fits_tabs.html"
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self.request = request
        self.query_params = {}
        # update_initial_cache(request, CACHE, FitsTabView)
    ...
    def get_context_data(self, **kwargs):
        ...
        # CACHE.get(self.request.session.session_key + "cbv")[FitsTabView] = context

        return context


@method_decorator(hx_request_required, name="dispatch")
class FitsCardView(ListView):
    """
    FitsCardView
    """
    ...
    template_name = "generic/fits_card.html"
    context_object_name = "queryset"
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        request = getattr(_thread_locals, "request", None)
        self.request = request
        # update_initial_cache(request, CACHE, FitsCardView)
    ...
    def get_context_data(self, **kwargs):
        ...
        # CACHE.get(self.request.session.session_key + "cbv")[FitsCardView] = context
        referrer = self.request.GET.get("referrer", "")
        if referrer:
            # Remove the protocol and domain part
            referrer = "/" + "/".join(referrer.split("/")[3:])
...
class FitsFormView(FormView):
    ...
    template_name = "generic/fits_form.html"
    ids_key: str = "instance_ids"
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        # update_initial_cache(request, CACHE, FitsFormView)
    ...
    def get_form(self, form_class=None):
        ...
        if not hasattr(self, "form"):
            ...
            if self.request.method == "GET":
                ...
                for dynamic_tuple in self.dynamic_create_fields:
                    ...
                    form.fields[field].widget.option_template_name = (
                        "fits_widgets/select_option.html",
                    )
            ...
            # CACHE.get(self.request.session.session_key + "cbv")[FitsFormView] = form
            self.form = form
        return self.form
...
@method_decorator(hx_request_required, name="dispatch")
class FitsNavView(TemplateView):
    """
    FitsNavView
    ...
    template_name = "generic/fits_nav.html"
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        request = getattr(_thread_locals, "request", None)
        self.request = request
        # update_initial_cache(request, CACHE, FitsNavView)
    ...
    def get_context_data(self, **kwargs):
        ...
        # CACHE.get(self.request.session.session_key + "cbv")[FitsNavView] = context
        return context
...
@method_decorator(hx_request_required, name="dispatch")
class FitsProfileView(DetailView):
    """
    GenericFitsProfileView
    """

    template_name = "generic/fits_profile_view.html"
    view_id: str = None
    filter_class: FilterSet = None
    push_url: str = None
    key_name: str = "pk"
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self.request = request
        self.ordered_ids_key = f"ordered_ids_{self.model.__name__.lower()}"
        # update_initial_cache(request, CACHE, FitsProfileView)
