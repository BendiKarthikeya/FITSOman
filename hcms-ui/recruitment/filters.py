...
class CandidateFilter(FilterSet):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Eagerly evaluate ModelChoiceField querysets to avoid connection pool timeout issues
        # during template rendering. Evaluate the queryset while connection is open.
        for field_name, field in form_fields.items():
            if hasattr(field, "queryset") and field.queryset is not None:
                try:
                    # Force evaluation of queryset to list to cache results
                    field.queryset._result_cache = list(field.queryset)
                except Exception:
...
class CandidateDatabaseFilter(FilterSet):
    """
    Filter set for the centralized Candidate Database & CV Management view.
    Supports searching by position, nationality, experience range, skills,
    project, and availability.
    """

    name = django_filters.CharFilter(
        field_name="name", lookup_expr="icontains", label=_("Candidate Name")
    )
    job_position_id = django_filters.ModelMultipleChoiceFilter(
        queryset=JobPosition.objects.all(), label=_("Position")
    )
    country = django_filters.CharFilter(
        field_name="country", lookup_expr="icontains", label=_("Nationality")
    )
    experience_min = django_filters.NumberFilter(
        field_name="experience_years", lookup_expr="gte", label=_("Min Experience (yrs)")
    )
    experience_max = django_filters.NumberFilter(
        field_name="experience_years", lookup_expr="lte", label=_("Max Experience (yrs)")
    )
    skills = django_filters.ModelMultipleChoiceFilter(
        field_name="recruitment_id__skills",
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        for field_name, field in self.form.fields.items():
            field.widget.attrs["id"] = str(uuid.uuid4())
