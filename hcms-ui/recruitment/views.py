...
from recruitment.views.linkedin import post_recruitment_in_linkedin
...
from recruitment.models import Candidate, Recruitment, RecruitmentApproval, Stage, StageNote
...
def is_recruitmentmanager(request, rec_id=False):
    ...
    return (employee in recruitment_obj.recruitment_managers.all()
            or user.is_superuser, employee.recruitment_set.all(), )
...
def create_employee_recruitment_approvals(recruitment_obj):
    """Create approval steps for an employee-raised recruitment."""
    if not recruitment_obj.raised_by:
        return []

    approvers = get_reporting_chain(recruitment_obj.raised_by)
    approvals = []
...
def recruitment(request):
        ...
        if form.is_valid():
            ...
            if recruitment_obj.raised_from_employee:
                ...
                if approvals:
                    ...
                    recipient = getattr(
                        first_approval.approver, "employee_user_id", None)
                    if recipient:
                        notify.send(
                    ...
                    recruitment_obj.save()
                    if recruitment_obj.raised_by and recruitment_obj.raised_by.employee_user_id:
                        notify.send(
                            request.user.employee_get,
                            recipient=[
                                recruitment_obj.raised_by.employee_user_id],
                            verb=_("Your recruitment request has been auto-approved."),
                            icon="checkmark-circle",
                            redirect=reverse("received-recruitments"),
                        )
            elif recruitment_obj.publish_in_linkedin and recruitment_obj.linkedin_account_id:
    ...
    return render(
        ...
        {
            "form": form,
            "raised_from_employee": (
                request.POST.get("raised_from_employee") == "True"
                or request.GET.get("raised_from_employee") == "True"
            ),
        },
    )
...
def received_recruitments(request):
    """Render employee-raised recruitments for HR and approval tracking."""
    pending_requests = Recruitment.objects.filter(
        raised_from_employee=True,
        approval_status="pending",
    ).order_by("-created_at")
    ready_to_publish_requests = Recruitment.objects.filter(
        raised_from_employee=True,
    ...
    return render(
        ...
        "recruitment/received_recruitments.html",
        {
            ...
            "ready_to_publish_requests": ready_to_publish_requests,
...
def publish_employee_recruitment(request, rec_id):
    ...
    if not recruitment_obj:
        messages.error(request, _("Recruitment is not ready to publish."))
        return redirect("received-recruitments")

    recruitment_obj.is_published = True
    recruitment_obj.save()

    if recruitment_obj.publish_in_linkedin and recruitment_obj.linkedin_account_id:
        post_recruitment_in_linkedin(
            request, recruitment_obj, recruitment_obj.linkedin_account_id
        )

    messages.success(request, _("Recruitment is now published."))
    return redirect("received-recruitments")
...
def approve_employee_recruitment(request, approval_id):
    approval = RecruitmentApproval.objects.select_related(
        "recruitment", "approver"
    ).get(id=approval_id)
    if not request.user.is_superuser and request.user.employee_get != approval.approver:
    ...
    pending_next = recruitment_obj.approvals.filter(status="pending").order_by(
        "sequence"
    ).first()
    if pending_next:
        recipient = getattr(pending_next.approver, "employee_user_id", None)
        if recipient:
            notify.send(
                request.user.employee_get,
                recipient=[recipient],
        ...
        if recruitment_obj.raised_by and recruitment_obj.raised_by.employee_user_id:
            notify.send(
                request.user.employee_get,
                recipient=[recruitment_obj.raised_by.employee_user_id],
                verb=_("Your recruitment request has been approved."),
                icon="checkmark-circle",
                redirect=reverse("received-recruitments"),
            )
    recruitment_obj.save()
    messages.success(request, _("Recruitment request approved."))
    return redirect("received-recruitments")
...
def remove_recruitment_manager(request, mid, rid):
    ...
    return render(request,
                  ...
                  {"data": paginator_qry(recruitment_queryset,
...
def recruitment_view(request):
    ...
    filter_obj = RecruitmentFilter(
        request.GET, queryset=Recruitment.objects.all())
    return render(
        request,
                ...
                if candidate_form.is_valid():
                    ...
                    with contextlib.suppress(Exception):
                        ...
                        users = [
                            employee.employee_user_id for employee in managers]
                        notify.send(
                            request.user.employee_get, recipient=users, verb=f"New candidate arrived on stage {
                                candidate_obj.stage_id.stage}", verb_ar=f"وصل مرشح جديد إلى المرحلة {
                                candidate_obj.stage_id.stage}", verb_de=f"Neuer Kandidat ist auf der Stufe {
                                candidate_obj.stage_id.stage} angekommen", verb_es=f"Nuevo candidato llegó a la etapa {
                                candidate_obj.stage_id.stage}", verb_fr=f"Nouveau candidat arrivé à l'étape {
                                candidate_obj.stage_id.stage}", icon="person-add", redirect=reverse("pipeline"), )
                    ...
                    with contextlib.suppress(Exception):
                        ...
                        notify.send(
                            ...
                            verb=f"You are chosen as a stage manager on the stage {
                                stage_obj.stage} in recruitment {
                                stage_obj.recruitment_id}",
                            verb_ar=f"لقد تم اختيارك كمدير مرحلة في المرحلة {
                                stage_obj.stage} في التوظيف {
                                stage_obj.recruitment_id}",
                            verb_de=f"Sie wurden als Bühnenmanager für die Stufe {
                                stage_obj.stage} in der Rekrutierung {
                                stage_obj.recruitment_id} ausgewählt",
                            verb_es=f"Has sido elegido/a como gerente de etapa en la etapa {
                                stage_obj.stage} en la contratación {
                                    stage_obj.recruitment_id}",
                            verb_fr=f"Vous avez été choisi(e) comme responsable de l'étape {
                                stage_obj.stage} dans le recrutement {
                                stage_obj.recruitment_id}",
                            icon="people-circle",
                            redirect=reverse("pipeline"),
                        )
    ...
    if request.POST:
        ...
        if form.is_valid():
            ...
            response = render(request,
                              "pipeline/form/recruitment_update.html",
                              {"form": form})
            return HttpResponse(
                response.content.decode("utf-8") +
                "<script>location.reload();</script>")
...
def candidate_stage_update(request, cand_id):
    ...
    if previous_stage == stage_obj:
        return JsonResponse(
            {"type": "info", "message": _("Sequence updated.")})
    # Here set the last updated schedule date on this stage if schedule exists
    # in history
    history_queryset = candidate_obj.candidate_history.filter(
        stage_id=stage_obj)
    schedule_date = None
...
def note_update(request, note_id):
    ...
    if request.POST:
        ...
        if form.is_valid():
            ...
            response = render(request,
                              "pipeline/pipeline_components/update_note.html",
                              {"form": form})
            return HttpResponse(
                response.content.decode("utf-8") +
                "<script>location.reload();</script>")
        ...
        if form.is_valid():
            ...
            rec_stages = (
                Stage.objects.filter(
                    recruitment_id=recruitment_obj,
                    is_active=True) .order_by("sequence") .last())
            ...
            with contextlib.suppress(Exception):
                ...
                notify.send(
                    ...
                    verb=f"Stage {stage_obj} is updated on recruitment {
                        stage_obj.recruitment_id}, You are chosen as one of the managers",
                    verb_ar=f"تم تحديث المرحلة {stage_obj} في التوظيف {
                        stage_obj.recruitment_id}، تم اختيارك كأحد المديرين",
                    verb_de=f"Stufe {stage_obj} wurde in der Rekrutierung {
                        stage_obj.recruitment_id} aktualisiert. Sie wurden als einer der Manager ausgewählt",
        ...
        if form.is_valid():
            ...
            if candidate_obj.stage_id is None:
                candidate_obj.stage_id = Stage.objects.filter(
                    recruitment_id=candidate_obj.recruitment_id, stage_type="initial").first()
...
def candidate_bulk_delete(request):
    ...
    for cand_id in ids:
        ...
        try:
            ...
            messages.success(
                request,
                _("%(candidate_obj)s deleted.") % {
                    "candidate": candidate_obj})
        except Exception as error:
            messages.error(
                request,
                _("You cannot delete %(candidate_obj)s") % {
                    "candidate": candidate_obj},
            )
            messages.error(request, error)
    ...
    if request.POST:
        ...
        if form.is_valid():
            ...
            if stages.filter(stage_type="applied").exists():
                candidate_obj.stage_id = stages.filter(
                    stage_type="applied").first()
            else:
...
def send_acknowledgement(request):
    ...
    with contextlib.suppress(Exception):
        ...
        if request:
            try:
                display_email_name = f"{
                    request.user.employee_get.get_full_name()} <{
                    request.user.employee_get.email}>"
...
def dashboard_pipeline(_):
    ...
    for rec in recruitment_obj:
        data = [
            stage_type_candidate_count(
                rec,
                type[0]) for type in Stage.stage_types]
        data_set.append(
            {
                "label": (
                    rec.title
...
def get_open_position(request):
    ...
    return JsonResponse(
        {"openPositions": job_info, "recruitmentInfo": rec_info})
...
def stage_sequence_update(request):
    ...
    for stage_id, seq in sequence_data.items():
        ...
        stage.save()
    return JsonResponse(
        {"type": "success", "message": "Stage sequence updated"})
