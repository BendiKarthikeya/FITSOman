...
class RecruitmentCreationForm(BaseModelForm):
    ...
    use_hierarchy = forms.BooleanField(
      required=False,
      label="Auto-assign managers from reporting hierarchy"
     )
    # Rendered as a toggle; True = External, False = Internal.
    posting_type = forms.BooleanField(
        required=False,
        label=_("Posting Type (Internal / External)"),
    )
    class Meta:
        """
        ...
        widgets = {
            ...
            "expat_allowed": forms.CheckboxInput(),
            "location": forms.TextInput(attrs={"class": "oh-input w-100", "placeholder": "e.g. Muscat"}),
        }
    ...
    def as_p(self, *args, **kwargs):
        ...
        """
        context = {"form": self}
        table_html = render_to_string("fits_form.html", context)
        return table_html
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Seed posting_type toggle initial value from existing instance, if any.
        if self.instance.pk:
            self.fields["posting_type"].initial = (self.instance.posting_type == "external")
    ...
    def clean(self):
        ...
        use_hierarchy = cleaned_data.get("use_hierarchy")
        managers = cleaned_data.get("recruitment_managers")

        if not use_hierarchy and not managers:
            self.add_error("recruitment_managers", "This field is required.")

        open_positions = cleaned_data.get("open_positions")

        # Toggle (BooleanField) → CharField value the model expects.
        cleaned_data["posting_type"] = "external" if cleaned_data.get("posting_type") else "internal"

        if cleaned_data["posting_type"] == "external" and not open_positions:
            raise forms.ValidationError(
                _("Job position is required if the recruitment is publishing.")
            )

        # Compute budget_available server-side from selected job positions' department.
        from recruitment.models import _is_privileged_department
        budget_available = False
        if open_positions:
            for jp in open_positions:
                dept = getattr(jp.department_id, "department", "") or ""
                if _is_privileged_department(dept):
                    budget_available = True
                    break
        cleaned_data["budget_available"] = budget_available

        if not budget_available and not (
            cleaned_data.get("budget_document") or self.files.get("budget_document")
        ):
            self.add_error(
                "budget_document",
                _("Budget document is required when no privileged department is selected."),
            )
...
class StageCreationForm(BaseModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        reload_queryset(self.fields)
        if not self.instance.pk:
            self.fields["stage_managers"] = FitsMultiSelectField(
                queryset=Employee.objects.filter(is_active=True),
                widget=FitsMultiSelectWidget(
                    filter_route_name="employee-widget-filter",
                    filter_class=EmployeeFilter,
    ...
    def clean(self):
        if isinstance(self.fields["stage_managers"], FitsMultiSelectField):
...
class ScheduleInterviewForm(BaseModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["interview_time"].required = False
...
class EmployeeRecruitmentForm(BaseModelForm):
    """
    Simplified form for employees to raise recruitment requests.
    Employees can only set basic info; managers and HR handle the rest.
    """
    ...
    def save(self, commit=True):
        instance = super().save(commit=False)
        instance.raised_from_employee = True
...
class CareersApplicationForm(forms.Form):
    """
    Public careers application form for job seekers
    """
    ...
    experience = forms.CharField(
        max_length=1000,
        required=True,
        label='Relevant Experience in this Role',
        widget=forms.Textarea(attrs={
            'class': 'form-control',
            'placeholder': 'Share your past experience relevant to this role...',
            'rows': 4,
        })
    )
    ...
    def clean_phone(self):
        ...
        if phone:
            # Use the same validation as Candidate.mobile
            pattern = r"^\+[0-9 ]+$|^[0-9 ]+$"
