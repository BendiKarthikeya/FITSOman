...
def application_form(request):
    ...
    source = request.GET.get("source", "application")
    ...
    if request.POST:
        ...
        if form.is_valid():
            ...
            # Set source (portal tracking)
            post_source = request.POST.get("source", "application")
            valid_sources = ("application", "software", "linkedin", "bayt", "naukrigulf", "other")
            candidate_obj.source = post_source if post_source in valid_sources else "application"
            ...
            # Run AI screening for portal-sourced applications
            try:
                from recruitment.views.views import _attempt_ai_screening
                application_text = (
                    f"Name: {candidate_obj.name}\n"
                    f"Email: {candidate_obj.email}\n"
                    f"Why Apply:\n{request.POST.get('why_apply', '')}\n"
                    f"Experience:\n{request.POST.get('experience', '')}"
                )
                _attempt_ai_screening(candidate_obj, recruitment, application_text)
            except Exception:
                pass
    ...
    return render(
        ...
        {"form": form, "recruitment": recruitment, "resume": resume_obj, "source": source},
    )
