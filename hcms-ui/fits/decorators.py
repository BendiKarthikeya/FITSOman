...
from fits.settings import BASE_DIR, TEMPLATES
...
def hr_login_required(func):
    """
    Decorator to check if user is HR staff
    """
    ...
    def wrapper(request, *args, **kwargs):
        ...
        if not request.user.is_authenticated:
            return redirect(reverse("hr-login"))
        ...
        try:
            ...
            hr_user = HRUser.objects.get(employee=employee, is_hr_staff=True)
            request.hr_user = hr_user
            return func(request, *args, **kwargs)
        except HRUser.DoesNotExist:
            messages.error(request, _("You are not authorized as HR staff."))
            return redirect(reverse('hr-login'))
        except Exception:
            messages.error(request, _("An error occurred during authorization."))
            return redirect(reverse("hr-login"))
...
DECORATOR_MAP = {
    ...
    "hr_login_required": hr_login_required,
}
