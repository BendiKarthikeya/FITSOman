    ...
    getCurrentLanguageCode(function (lang) {
        ...
        if (ids.length === 0) {
            ...
            if (takeAction === "delete") {
                ...
                Swal.fire({
                    text: deleteMessages[lang] || deleteMessages.en,
                    icon: "warning",
                    ...
                    if (result.isConfirmed) {
                        $("#bulkDeleteDocument").attr("hx-vals", JSON.stringify({ ids })).click();
                    }
                });
            } else {
                const triggerId =
...
function updateSelectedDocumentsCount() {
    var count = $(".all-documents-row:checked").length;
    if (count > 0) {
        $("#selectedDocuments").text("Selected Documents (" + count + ")").show();
    } else {
        $("#selectedDocuments").hide();
    }
}
