...
from fits.models import FitsModel, has_xss, upload_path
...
class Employee(models.Model):
    ...
    work_permit_number = models.CharField(max_length=50, blank=True, null=True, verbose_name=_("Work Permit Number"))
    last_appraisal_date = models.DateField(null=True, blank=True, verbose_name=_("Last Appraisal Date"))
    last_appraisal_rating = models.CharField(max_length=50, blank=True, null=True, verbose_name=_("Last Appraisal Rating"))
    ...
    objects = FitsCompanyManager(related_company_field="employee_work_info__company_id")
    ...
    def get_archive_condition(self):
        ...
        if apps.is_installed("onboarding"):
            OnboardingStage = get_fits_model_class("onboarding", "onboardingstage")
            OnboardingTask = get_fits_model_class("onboarding", "onboardingtask")
        ...
        if apps.is_installed("recruitment"):
            Recruitment = get_fits_model_class("recruitment", "recruitment")
            Stage = get_fits_model_class("recruitment", "stage")
    ...
    def check_online(self):
        ...
        """
        if apps.is_installed("attendance"):
            Attendance = get_fits_model_class("attendance", "attendance")
            ...
            if request is not None:
                if (
                    ...
                    yesterday = today - timedelta(days=1)
                    # Use a set so membership checks are O(1) and don't
                    # re-execute the queryset on every `in` check.
...
class EmployeeSalaryHistory(models.Model):
    """Immutable log of every salary change for an employee."""
    ...
    basic_salary = models.DecimalField(max_digits=12, decimal_places=2, verbose_name=_("Basic Salary"))
    gross_salary = models.DecimalField(max_digits=12, decimal_places=2, null=True, blank=True, verbose_name=_("Gross Salary"))
    increment_reason = models.CharField(max_length=200, blank=True, verbose_name=_("Reason"))
    recorded_by = models.ForeignKey(
        "Employee",
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        related_name="salary_records_made",
    ...
    def __str__(self):
        return f"{self.employee} — {self.basic_salary} from {self.effective_date}"
...
class EmployeeBankDetails(FitsModel):
    """
...
class PolicyMultipleFile(FitsModel):
    """
...
class DisciplinaryAction(FitsModel):
    """
    Disciplinary model
    """
...
class EmployeeGeneralSetting(FitsModel):
    """
...
class ProfileEditFeature(FitsModel):
    """
    ProfileEditFeature
    """
...
# Import organization chart models
