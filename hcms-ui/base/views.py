...
from fits.fits_settings import (
    APPS,
    DB_INIT_PASSWORD,
...
from fits.methods import get_fits_model_class, remove_dynamic_url
from fits_audit.forms import HistoryTrackingFieldsForm
from fits_audit.models import AccountBlockUnblock, AuditTag, HistoryTrackingFields
...
def initialize_database_condition():
    ...
    cached = cache.get("db_init_condition")
    ...
    if not init_database:
        ...
        for user in superusers:
            if hasattr(user, "employee_get"):
                init_database = False
                break
    # Cache for 60 seconds (only changes during initial setup)
    cache.set("db_init_condition", init_database, 60)
...
def initialize_database(request):
    ...
    if initialize_database_condition():
        ...
        return render(request, "initialize_database/fits_user.html")
    else:
        return redirect("/")
...
def initialize_database_user(request):
    ...
    if request.method == "POST":
        ...
        if password != confirm_password:
            return render(request, "initialize_database/fits_user_signup.html")
        ...
        return render(
            request,
            "initialize_database/fits_company.html",
...
def initialize_database_company(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(
                request,
                "initialize_database/fits_department.html",
                {"form": DepartmentForm(initial={"company_id": company})},
            )
    return render(request, "initialize_database/fits_company.html", {"form": form})
...
def initialize_database_department(request):
    ...
    return render(
        request,
        "initialize_database/fits_department_form.html",
...
def initialize_database_job_position(request):
    ...
    if request.method == "POST":
        ...
        return render(
            request,
            "initialize_database/fits_job_position_form.html",
            {
                "form": form,
    ...
    return render(
        request,
        "initialize_database/fits_job_position.html",
...
def login_user(request):
    """
    Handles user login and authentication with Neon DB integration.
    """
    ...
    if request.method == "POST":
        ...
        # Verify Neon DB connection before authentication
        neon_db_status = verify_neon_db_connection()
        ...
        # Add Neon DB status to success message
        if neon_db_status:
            messages.success(request, _("Login successful. Neon DB connection active."))
        else:
            messages.warning(
                request, _("Login successful. Neon DB connection issue detected.")
            )
        ...
        if params and next_url != "/":
            next_url += f"?{params}"

        if next_url != "/":
            return redirect(next_url)
        return redirect(_get_default_dashboard_url(user))

    # Check Neon DB status for the login page
    neon_db_status = None  # Loaded asynchronously by the template via /neon-db-status/
    ...
    return render(
        ...
        "modern_login.html",
        {
            ...
            "neon_db_status": neon_db_status,
        },
    )


def neon_db_status_api(request):
    """Lightweight JSON endpoint for async DB status check from the login page."""
    from django.http import JsonResponse

    status = verify_neon_db_connection()
    return JsonResponse({"active": status})


def verify_neon_db_connection():
    """
    Returns True if connection is successful, False otherwise.
    Cached for 30 seconds to avoid Neon cold-start latency on every page load.
    """
    from django.core.cache import cache

    cached = cache.get("neon_db_status")
    if cached is not None:
        return cached
    try:
        ...
        result = False
    cache.set("neon_db_status", result, 30)
    return result
...
def _get_user_roles(user):
    ...
    if employee is not None:
        is_hr_staff = HRUser.objects.filter(
            employee=employee, is_hr_staff=True
        ).exists()

    return {
        "employee": employee,
        "is_admin_staff": is_admin_staff,
        "is_hr_staff": is_hr_staff,
    }


def _get_default_dashboard_url(user):
    user_roles = _get_user_roles(user)
    if user_roles["is_admin_staff"]:
        return reverse("home-page")
    if user_roles["is_hr_staff"]:
        return reverse("hr-dashboard")
    return "/"


def _build_module_cards(request, module_keys):
    icon_map = {
        "employee": "people",
        "leave": "calendar",
        ...
        "expenses": "wallet",
        "helpdesk": "headset",
        "omani_compliance": "shield-checkmark",
        "fits_audit": "document-text",
        "learning": "school",
        "talent": "star",
        "onboarding": "rocket",
        "offboarding": "exit",
    ...
    details = {
        "employee": {
            ...
            "description": _("Manage employee records and information"),
            "url_name": "employee-view",
        },
        "leave": {
            "title": _("Leave Management"),
            "description": _("Handle leave requests and approvals"),
            "url_name": "leave-dashboard",
        },
        "attendance": {
            "title": _("Attendance Management"),
            "description": _("Track and manage employee attendance"),
            "url_name": "attendance-dashboard",
        },
        "payroll": {
            "title": _("Payroll Management"),
            "description": _("Manage salaries and compensation"),
            "url_name": "view-payroll-dashboard",
        },
        "recruitment": {
            "title": _("Recruitment"),
            "description": _("Manage job posts and candidates"),
            "url_name": "recruitment-dashboard",
        },
        "asset": {
            "title": _("Asset Management"),
            "description": _("Track equipment and assets"),
            "url_name": "asset-dashboard",
        },
        "project": {
            "title": _("Project"),
            "description": _("Manage projects and tasks"),
            "url_name": "project-dashboard-view",
        },
        "report": {
            "title": _("Reports"),
            "description": _("View analytics and reports"),
            "url_name": "recruitment-report",
        },
        "pms": {
            "title": _("Performance"),
            "description": _("Manage objectives, reviews and feedback"),
            "url_name": "dashboard-view",
        },
        "expenses": {
            "title": _("Expenses & Travel"),
            "description": _("Manage travel and expense claims"),
            "url_name": "expenses:dashboard",
        },
        "helpdesk": {
            "title": _("Help Desk"),
            "description": _("Manage support tickets and FAQs"),
            "url_name": "ticket-view",
        },
        "omani_compliance": {
            "title": _("Omani Compliance"),
            "description": _("Manage labor and tax compliance"),
            "url_name": "omani_compliance:omani_config",
        },
        "fits_audit": {
            "title": _("Audit & Compliance"),
            "description": _("Review audit dashboards and controls"),
            "url_name": "fits_audit:audit-dashboard",
        },
        "learning": {
            "title": _("Learning & Development"),
            "description": _("Manage courses, skills and training"),
            "url_name": "learning:dashboard",
        },
        "talent": {
            "title": _("Talent & Succession"),
            "description": _("Manage succession and talent profiles"),
            "url_name": "talent:dashboard",
        },
        "onboarding": {
            "title": _("Onboarding"),
            "description": _("Manage onboarding and pre-boarding"),
            "url_name": "onboarding-view",
        },
        "offboarding": {
            "title": _("Offboarding"),
            "description": _("Manage exits and resignation workflows"),
            "url_name": "offboarding-dashboard",
        },
    }
    ...
    for key in module_keys:
        ...
        if not detail:
            continue
        try:
            redirect_url = reverse(detail["url_name"])
        except Exception:
            redirect_url = "/"
        module_cards.append(
            {
...
def hr_dashboard(request):
    ...
    if not user_roles["is_hr_staff"]:
        if user_roles["is_admin_staff"]:
            return redirect("home-page")
        messages.error(request, _("You are not authorized to access HR Dashboard."))
        return redirect("login")

    hr_user = HRUser.objects.filter(
        employee=user_roles["employee"], is_hr_staff=True
    ).first()
    accessible_modules = hr_user.get_accessible_modules() if hr_user else []
    hr_module_order = [
        "learning",
        "talent",
        "recruitment",
    ...
    module_cards = _build_module_cards(
        request,
        [module for module in hr_module_order if module in accessible_modules],
    )
    ...
    return render(
        ...
        "hr/hr_dashboard.html",
        {
            ...
            "dashboard_title": _("HR Modules"),
            "dashboard_heading": _("HR Administration Dashboard"),
            "dashboard_subtitle": _("Welcome to the HR Management Portal"),
        },
    )
...
def superuser_dashboard(request):
    ...
    if not user_roles["is_admin_staff"]:
        return redirect(_get_default_dashboard_url(request.user))

    superuser_module_order = [
        "attendance",
        "employee",
        "leave",
        "project",
    ...
    module_cards = _build_module_cards(request, superuser_module_order)
    ...
    return render(
        ...
        "hr/hr_dashboard.html",
        {
            "employee": user_roles["employee"],
            "hr_user": None,
            "accessible_modules": superuser_module_order,
            "module_cards": module_cards,
            "dashboard_title": _("SuperUser Modules"),
            "dashboard_heading": _("SuperUser Dashboard"),
            "dashboard_subtitle": _("Manage core modules with superuser access"),
        },
    )
...
def hr_login(request):
    """
    Backward-compatible alias for the unified login page.
    """
...
class FitsPasswordResetView(PasswordResetView):
    """
    Fits View for Reset Password
    """
...
class EmployeePasswordResetView(PasswordResetView):
    """
    Fits View for Employee Reset Password
    """
...
def two_factor_auth(request):
    ...
    if not fits_apps.TWO_FACTORS_AUTHENTICATION:
        return redirect("/")
...
def home(request):
    ...
    if user.employee_get.dob is not None:
        is_birthday = (
...
def object_delete(request, obj_id, **kwargs):
    ...
    try:
        ...
        (
            messages.error(
                request,
                _("This {} is already in use for {}.").format(
                    instance, model_names_str
                ),
            ),
        )

    if apps.is_installed("pms") and redirect_path == "/pms/filter-key-result/":
        KeyResult = get_fits_model_class(app_label="pms", model="keyresult")
...
def mail_server_test_email(request):
    ...
    image_path = path.join(settings.STATIC_ROOT, "images/ui/fits-logo.png")
    company_name = "FITS HCMS"
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            subject = _("Test mail from FITS HCMS")
...
@login_required
@permission_required("base.view_fitsmailtemplate")
...
@login_required
@hx_request_required
@permission_required("base.change_fitsmailtemplate")
def view_mail_template(request, obj_id):
    """
    This method is used to display the template/form to edit
    """
    template = FitsMailTemplate.objects.get(id=obj_id)
...
@login_required
@hx_request_required
@permission_required("base.add_fitsmailtemplate")
...
@login_required
@permission_required("base.delete_fitsmailtemplate")
def delete_mail_templates(request):
    ids = request.GET.getlist("ids")
    FitsMailTemplate.objects.filter(id__in=ids).delete()
...
def employee_shift_view(request):
    ...
    if apps.is_installed("attendance"):
        GraceTime = get_fits_model_class(app_label="attendance", model="gracetime")
...
def general_settings(request):
    ...
    if apps.is_installed("payroll"):
        PayrollSettings = get_fits_model_class(
            app_label="payroll", model="payrollsettings"
        )
        EncashmentGeneralSettings = get_fits_model_class(
...
@login_required
@permission_required("fits_audit.change_accountblockunblock")
...
@login_required
@permission_required("fits_audit.view_audittag")
...
@login_required
@hx_request_required
@permission_required("fits_audit.add_audittag")
...
@login_required
@hx_request_required
@permission_required("fits_audit.change_audittag")
...
@login_required
def get_fits_installed_apps(request):
...
try:
    ...
    InvalidToken = TokenError = Exception
    UntypedToken = None


def is_jwt_token_valid(auth_header):
    if JWTAuthentication is None or UntypedToken is None:
        return None
...
def feature_stub_view(request, **kwargs):
    from django.shortcuts import render

    # Provide the path title and avoid 404
    context = {"title": request.path}
    return render(request, "feature_stub.html", context)


@login_required
def headcount_planning(request):
    return render(request, "base/headcount_planning.html")
