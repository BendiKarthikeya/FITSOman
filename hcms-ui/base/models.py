...
from fits import fits_middlewares
...
class JobPosition(FitsModel):
    ...
    objects = FitsCompanyManager("department_id__company_id")
...
class JobRole(FitsModel):
    ...
    objects = FitsCompanyManager("job_position_id__department_id__company_id")
...
class RotatingWorkType(FitsModel):
    """
    ...
    objects = FitsCompanyManager("employee_id__employee_work_info__company_id")
...
class RotatingWorkTypeAssign(FitsModel):
    """
...
class EmployeeShift(FitsModel):
    ...
    objects = FitsCompanyManager("employee_shift__company_id")
...
class EmployeeShiftSchedule(FitsModel):
    """
    ...
    auto_punch_out_time = models.TimeField(
        ...
        help_text=_(
            "Time at which the fits will automatically check out the employee attendance if they forget."
        ),
    )
    company_id = models.ManyToManyField(Company, blank=True, verbose_name=_("Company"))

    objects = FitsCompanyManager("shift_id__employee_shift__company_id")
...
class RotatingShiftAssign(FitsModel):
    """
...
class WorkTypeRequest(FitsModel):
    """
    ...
    def clean(self):
        request = getattr(fits_middlewares._thread_locals, "request", None)
...
class WorkTypeRequestComment(FitsModel):
    """
...
class ShiftRequestComment(FitsModel):
    """
...
class FitsMailTemplate(FitsModel):
...
class DynamicEmailConfiguration(FitsModel):
    """
...
class MultipleApprovalCondition(FitsModel):
...
class AnnouncementComment(FitsModel):
    """
...
class TrackLateComeEarlyOut(FitsModel):
...
class PenaltyAccounts(FitsModel):
    """
...
class HRUser(models.Model):
    """
    Model to identify and manage HR staff members
    """
    ...
    is_hr_staff = models.BooleanField(
        default=True,
        verbose_name=_("Is HR Staff"),
        help_text=_("Designate this employee as HR staff"),
    )
    can_view_all_companies = models.BooleanField(
        default=False,
        verbose_name=_("Can View All Companies"),
        help_text=_("Allow HR staff to view data across all companies"),
    )
    can_manage_employees = models.BooleanField(
        default=True, verbose_name=_("Can Manage Employees")
    )
    can_manage_leaves = models.BooleanField(
        default=True, verbose_name=_("Can Manage Leaves")
    )
    can_manage_attendance = models.BooleanField(
        default=True, verbose_name=_("Can Manage Attendance")
    )
    can_manage_payroll = models.BooleanField(
        default=True, verbose_name=_("Can Manage Payroll")
    )
    can_manage_recruitment = models.BooleanField(
        default=True, verbose_name=_("Can Manage Recruitment")
    )
    can_manage_assets = models.BooleanField(
        default=True, verbose_name=_("Can Manage Assets")
    )
    can_manage_biometric = models.BooleanField(
        default=True, verbose_name=_("Can Manage Biometric")
    )
    can_manage_pms = models.BooleanField(
        default=True, verbose_name=_("Can Manage Performance")
    )
    can_manage_reports = models.BooleanField(
        default=True, verbose_name=_("Can Manage Reports")
    )
    can_manage_onboarding = models.BooleanField(
        default=True, verbose_name=_("Can Manage Onboarding")
    )
    can_manage_offboarding = models.BooleanField(
        default=True, verbose_name=_("Can Manage Offboarding")
    )
    can_manage_projects = models.BooleanField(
        default=True, verbose_name=_("Can Manage Projects")
    )
    can_manage_omani_compliance = models.BooleanField(
        default=True, verbose_name=_("Can Manage Omani Compliance")
    )
    can_manage_expenses = models.BooleanField(
        default=True, verbose_name=_("Can Manage Expenses")
    )
    can_manage_learning = models.BooleanField(
        default=True, verbose_name=_("Can Manage Learning")
    )
    can_manage_fits_audit = models.BooleanField(
        default=True, verbose_name=_("Can Manage Audit")
    )
    can_manage_helpdesk = models.BooleanField(
        default=True, verbose_name=_("Can Manage Helpdesk")
    )
    can_manage_talent = models.BooleanField(
        default=True, verbose_name=_("Can Manage Talent")
    )
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    def __str__(self):
        return f"HR - {self.employee.employee_first_name} {self.employee.employee_last_name}"

    def get_accessible_modules(self):
        """
        Return list of modules this HR user can access
        """
        modules = []
        if self.can_manage_employees:
            modules.append("employee")
        if self.can_manage_leaves:
            modules.append("leave")
        if self.can_manage_attendance:
            modules.append("attendance")
        if self.can_manage_payroll:
            modules.append("payroll")
        if self.can_manage_recruitment:
            modules.append("recruitment")
        if self.can_manage_assets:
            modules.append("asset")
        if self.can_manage_biometric:
            modules.append("biometric")
        if self.can_manage_pms:
            modules.append("pms")
        if self.can_manage_reports:
            modules.append("report")
        if self.can_manage_onboarding:
            modules.append("onboarding")
        if self.can_manage_offboarding:
            modules.append("offboarding")
        if self.can_manage_projects:
            modules.append("project")
        if self.can_manage_omani_compliance:
            modules.append("omani_compliance")
        if self.can_manage_expenses:
            modules.append("expenses")
        if self.can_manage_learning:
            modules.append("learning")
        if self.can_manage_fits_audit:
            modules.append("fits_audit")
        if self.can_manage_helpdesk:
            modules.append("helpdesk")
        if self.can_manage_talent:
            modules.append("talent")
        return modules

    def save(self, *args, **kwargs):
        """
        Override save to automatically set employee's user as staff when HR user is created
        """
        super().save(*args, **kwargs)

        # Grant staff status to the employee's user
        if self.is_hr_staff and self.employee and self.employee.employee_user_id:
...
# Re-export integration models so Django discovers them
from base.models_integrations import MailboxIntegration  # noqa: E402,F401
